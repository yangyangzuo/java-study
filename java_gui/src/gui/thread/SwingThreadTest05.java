...
import javax.swing.Timer;
	...
	/**
	 * 我们冲一个非常简单的，但是非常耗时的任务开始,当前这个applet加载一些图形文件用于形成动画
	 * 如果在initial thread中加载图形文件，在gui出现之前可能有很大延迟.
	 * 如果从event dispatc thread加载图形文件，gui可能短暂无响应
	 * 为了避免这些问题,当前applet在initial threads创建并执行了一个SwingWorker的实例
	 * 这个对象的doInBackground方法在一个work thread中执行，加载图像到imageicon数组中，然后返回这个数组的引用
	 * 当这个方法执行完了以后,在event dispatch thread上执行,把引用的数组传递给imgs,这可以让当前applet立即构建gui，无需等待图片加载完成
	 * 
	 */
	 ...
	 *
	 */
	public class SwingThreadTest05 extends JApplet
